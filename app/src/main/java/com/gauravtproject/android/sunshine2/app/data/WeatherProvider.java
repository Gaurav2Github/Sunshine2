...
/**
 * Created by Gaurav Tandon on 6/03/2015.
 */
public class WeatherProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        final int match = sUriMatcher.match(uri);
        int returnDeletedRows;
        selection = null == selection ? "1" : selection;
        switch (match){
            case WEATHER:   {
                returnDeletedRows = db.delete(WeatherContract.WeatherEntry.TABLE_NAME,selection,selectionArgs);
                break;
            }
            case LOCATION:  {
                returnDeletedRows = db.delete(WeatherContract.LocationEntry.TABLE_NAME,selection,selectionArgs);
                break;
            }
    ...
    public int update(
        ...
        switch (match){
            case WEATHER:   {
                returnUpdatedNo =  db.update(WeatherContract.WeatherEntry.TABLE_NAME, values, selection,selectionArgs);
                break;
            }

            case LOCATION:  {
                returnUpdatedNo = db.update(WeatherContract.LocationEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            default:    {
                throw new UnsupportedOperationException("Unknown Uri " + uri);
        ...
        if(returnUpdatedNo!=0)
            getContext().getContentResolver().notifyChange(uri, null);

        return returnUpdatedNo;
    }
